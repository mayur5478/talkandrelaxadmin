...
function ListenerBlock({ search }) {
  ...
  const [unblockListener, { isLoading: isUnblockLoading }] = useUnblockListenerMutation();
  ...
  const confirmUnblock = async () => {
    try {
      await unblockListener(selectedUnblock).unwrap();
      refetch();
    } catch (err) {
  ...
  if (error)     return <div className="tw-p-4"><ErrorBanner title="Failed to load blocked listeners" /></div>;
      ...
      <Table>
        ...
        <TBody>
          {rows.length === 0 ? (
            <TR isLast>
              <Td colSpan={4}>
                <EmptyState title="No blocked listeners" description="No listeners have been blocked yet." />
              </Td>
            </TR>
          ) : (
            rows.map((item, index) => (
